...
[ "$VERSION" = 1.22.0 ] && ok || bad "VERSION wanted 1.22.0 got $VERSION"
...
[ "$ver" = 1.22.0 ] && ok || bad "crucible --version wanted 1.22.0 got $ver"
ver=$("$STAGE/crucible" -V 2>"$ERR") || ver=
[ "$ver" = 1.22.0 ] && ok || bad "crucible -V wanted 1.22.0 got $ver"

# Wrapper execs the sibling binary (not POSIX ./crucible).
wrap=$("$WM" --version 2>"$ERR") || wrap=
[ "$wrap" = 1.22.0 ] && ok || bad "wm.sh --version (wrapper exec) wanted 1.22.0 got $wrap err=$(cat "$ERR")"
