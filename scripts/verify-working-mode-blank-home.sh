...
# Falsifier-first: RED when adapters/docs/VERSION 1.7.1 pin are absent.
...
if [ "$VERSION" != 1.7.1 ]; then
  printf 'RED VERSION is %s, want 1.7.1\n' "$VERSION" >&2
  exit 1
fi

if ! grep -q '^## \[1.7.1\] - 2026-09-10$' "$HERE/CHANGELOG.md"; then
  printf 'RED CHANGELOG.md missing ## [1.7.1] - 2026-09-10\n' >&2
  exit 1
fi
...
# Tarball pin: package-release archives git REF (HEAD must record VERSION 1.7.1).
PKG_OUT="$BASE/pkg"
mkdir -p "$PKG_OUT"
...
if [ -n "$EXTRACT" ]; then
  ...
  if [ -f "$AD/.crucible/work/ENGINE-SOURCE" ]; then
    grep -q '^version: 1.7.1$' "$AD/.crucible/work/ENGINE-SOURCE" \
      && ok || bad 'ENGINE-SOURCE version is not 1.7.1'
  else
