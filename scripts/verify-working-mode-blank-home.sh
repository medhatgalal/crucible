...
# Falsifier-first: RED when adapters/docs/VERSION 1.19.0 pin are absent.
...
if [ "$VERSION" != 1.19.0 ]; then
  printf 'RED VERSION is %s, want 1.19.0\n' "$VERSION" >&2
  exit 1
fi
...
# Tarball pin: package-release archives git REF (HEAD must record VERSION 1.19.0).
PKG_OUT="$BASE/pkg"
mkdir -p "$PKG_OUT"
...
if [ -n "$EXTRACT" ]; then
  ...
  [ "$_ad_ver" = 1.19.0 ] && ok || bad "adopted crucible --version wanted 1.19.0 got $_ad_ver"
  ...
  if [ -f "$AD/.crucible/work/ENGINE-SOURCE" ]; then
    grep -q '^version: 1.19.0$' "$AD/.crucible/work/ENGINE-SOURCE" \
      && ok || bad 'ENGINE-SOURCE version is not 1.19.0'
  else
...
if [ -n "$AD" ] && [ -f "$AD/.crucible/work/wm.sh" ]; then
  ...
  [ "$wrap_ver" = 1.19.0 ] && ok || bad "adopted wm.sh --version wanted 1.19.0 got $wrap_ver err=$(cat "$ERR")"
