...
Operator extra-proof against **1.15.2**. Fixture CLIs for A/B/blank-home.
...
| A | Example A LOW hello `loop` | `CLOSED PASS`, `hello` | `verify-working-mode-quickstart.sh` **46/0** on 1.15.2 |
...
| C | Vague IDEA + live specifier | unattended hello `CLOSED PASS` | `/tmp/wm-butter-hello3.p4cSeA` on 1.15.1; no operator git after `go` |
| NO-BUILD | Product already matches falsifier | `CLOSED NO-BUILD`; PASS after no-build red refused | kernel **438/0** |
| E5 | Several modules, `depends_on`, one loop | 3 CLOSED slices | `verify-working-mode-blank-home.sh` **102/0** |
| Live | grok + kiro-cli + codex, health-check IDEA | `CLOSED PASS`, four distinct PIDs, `CROSS-FAMILY` | `verify-working-mode-live.sh` **53/0** on **1.15.2**; kiro host-HOME pong rc 0 |
| T | Unattended tinytag brownfield `TINYTAG_DIR` | `CLOSED PASS`, `CROSS-FAMILY`, `python3 tests/test_tinytag.py` rc 0 | `/tmp/wm-butter-tinytag3.9tj55V`; 0 operator commits after `go` |
| QS-home | empty HOME stays skill-free | pass | quickstart + blank-home + kernel |

Receipts: `docs/examples/working-mode/receipts/`.
